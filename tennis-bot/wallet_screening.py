...
def fetch_recent_buys(address: str, weeks: int, max_count: int,
    ...
    not just tennis) to collect every BUY trade within the last `weeks`
    weeks, stopping as soon as more than `max_count` have been found.
    ...
    backtest.py on the allsports variant of this bot — see MAX_BUYS
    above), the 500 most recent trades cover under 4 days, nowhere near
    the 10-week lookback window, so its true volume never surfaces and
    MAX_BUYS can never trigger. This paginates properly instead, and
    checks TOTAL activity across every category (not just tennis) since
    that's the actual signature of a market-maker/arb bot — its
    tennis-specific volume alone can look perfectly modest even while its
    overall activity is enormous.
...
def screen_wallet(address: str, min_buys: int = None, min_win_rate: float = None,
    ...
    sport_buys = [t for t in all_recent_buys if _sport_filter.is_match(t.get("slug") or "")]
    meets_activity_bar = len(sport_buys) >= min_buys

    # Win-rate computation resolves every unique market the wallet touched
    # — for a wallet with many unique markets that's real work for a
